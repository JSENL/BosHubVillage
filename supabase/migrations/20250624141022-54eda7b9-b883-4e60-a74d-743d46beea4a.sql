...
CREATE POLICY "Anyone can view approved local resources submissions" ON public.local_services_nonprofits_submissions 
  FOR SELECT USING (status = 'approved');
CREATE POLICY "Users can view their own local resources submissions" ON public.local_services_nonprofits_submissions 
  FOR SELECT USING (auth.uid() = submitted_by);
CREATE POLICY "Authenticated users can create local resources submissions" ON public.local_services_nonprofits_submissions 
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND auth.uid() = submitted_by);
CREATE POLICY "Users can update their own pending local resources submissions" ON public.local_services_nonprofits_submissions 
  FOR UPDATE USING (auth.uid() = submitted_by AND status = 'pending');

-- RLS policies for local_services_nonprofits_comments
CREATE POLICY "Anyone can view local resources comments" ON public.local_services_nonprofits_comments 
  FOR SELECT USING (true);
CREATE POLICY "Authenticated users can create local resources comments" ON public.local_services_nonprofits_comments 
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL AND auth.uid() = user_id);
CREATE POLICY "Users can update their own local resources comments" ON public.local_services_nonprofits_comments 
  FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own local resources comments" ON public.local_services_nonprofits_comments 
