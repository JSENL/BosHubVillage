  ...
  description_translations = jsonb_set(
    jsonb_set(
      jsonb_set(description_translations, '{pt}', 
        CASE 
          WHEN description = 'Local New England store that sells Local Artist arts and crafts. Have classes and other community-inspired events.' THEN '"Loja local de New England que vende artes e artesanatos de artistas locais. Oferece aulas e outros eventos inspirados na comunidade."'::jsonb
          ...
          WHEN description = 'Popular Vietnamese restaurant known for its authentic pho and traditional Vietnamese dishes.' THEN '"Restaurante vietnamita popular conhecido por seu pho autêntico e pratos tradicionais vietnamitas."'::jsonb
      ...
      '{it}', 
        CASE 
          ...
          WHEN description = 'Popular Vietnamese restaurant known for its authentic pho and traditional Vietnamese dishes.' THEN '"Ristorante vietnamita popolare noto per il suo pho autentico e i piatti tradizionali vietnamiti."'::jsonb
          ELSE ('"' || description || '"')::jsonb
        END
    ),
    '{ar}', 
      CASE 
        ...
        WHEN description = 'Family-owned pizzeria serving pizza, pasta, and subs.' THEN '"بيتزيريا عائلية تقدم البيتزا والمعكرونة والسندويشات."'::jsonb
        WHEN description = 'Popular Vietnamese restaurant known for its authentic pho and traditional Vietnamese dishes.' THEN '"مطعم فيتنامي شهير معروف بالفو الأصيل والأطباق الفيتنامية التقليدية."'::jsonb
        ELSE ('"' || description || '"')::jsonb
      END
  ),
