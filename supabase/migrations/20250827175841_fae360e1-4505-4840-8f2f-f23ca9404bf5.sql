...
SET 
  ...
  description_translations = jsonb_set(
    jsonb_set(
      jsonb_set(description_translations, '{pt}', 
        CASE 
          ...
          WHEN description = 'Community organization offering housing, food programs, and a social enterprise café.' THEN '"Organização comunitária oferecendo habitação, programas de alimentação e um café de empreendimento social."'::jsonb
          WHEN description = 'Freedom House has been a beacon of change, championing educational equity and economic opportunities for Black, Brown, and immigrant youth across our communities.' THEN '"A Casa da Liberdade tem sido um farol de mudança, defendendo a equidade educacional e oportunidades econômicas para jovens negros, pardos e imigrantes em nossas comunidades."'::jsonb
      ...
      '{it}', 
        CASE 
          ...
          WHEN description = 'Freedom House has been a beacon of change, championing educational equity and economic opportunities for Black, Brown, and immigrant youth across our communities.' THEN '"Freedom House è stato un faro di cambiamento, sostenendo l''equità educativa e le opportunità economiche per i giovani neri, ispanici e immigrati nelle nostre comunità."'::jsonb
    ...
    '{ar}', 
      CASE 
        WHEN description = 'Be your child''s First Teacher!' THEN '"كن أول معلم لطفلك!"'::jsonb
        ...
        WHEN description = 'Education Program that works with students to prepare for Exam Schools, offers one-on-one tutoring and other classes.' THEN '"برنامج تعليمي يعمل مع الطلاب لإعدادهم لمدارس الامتحانات، يقدم دروساً خصوصية فردية وصفوف أخرى."'::jsonb
        ELSE ('"' || description || '"')::jsonb
      END
  ),
