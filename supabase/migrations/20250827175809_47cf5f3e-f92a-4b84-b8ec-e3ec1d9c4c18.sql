  ...
  description_translations = jsonb_set(
    jsonb_set(
      jsonb_set(description_translations, '{pt}', 
        CASE 
          ...
          WHEN description = 'Community organization offering housing, food programs, and a social enterprise café.' THEN '"Organização comunitária que oferece habitação, programas alimentares e um café de empreendimento social."'::jsonb
          ...
          WHEN description = 'Education Program that works with students to prepare for Exam Schools, offers one-on-one tutoring and other classes.' THEN '"Programa educacional que trabalha com estudantes para prepará-los para escolas de exame, oferece tutoria individual e outras aulas."'::jsonb
      ...
      '{it}', 
        CASE 
          ...
          WHEN description = 'Freedom House has been a beacon of change, championing educational equity and economic opportunities for Black, Brown, and immigrant youth across our communities.' THEN '"Freedom House è stata un faro di cambiamento, promuovendo l''equità educativa e le opportunità economiche per i giovani neri, marroni e immigrati nelle nostre comunità."'::jsonb
          WHEN description = 'Promotes urban farming, food access, and community workshops.' THEN '"Promuove l''agricoltura urbana, l''accesso al cibo e i laboratori comunitari."'::jsonb
    ...
    '{ar}', 
      CASE 
        ...
        WHEN description = 'Community organization offering housing, food programs, and a social enterprise café.' THEN '"منظمة مجتمعية تقدم الإسكان وبرامج الطعام ومقهى للمؤسسات الاجتماعية."'::jsonb
        WHEN description = 'Freedom House has been a beacon of change, championing educational equity and economic opportunities for Black, Brown, and immigrant youth across our communities.' THEN '"كان بيت الحرية منارة للتغيير، يدعم المساواة التعليمية والفرص الاقتصادية للشباب الأسود والبني والمهاجر في مجتمعاتنا."'::jsonb
        WHEN description = 'Promotes urban farming, food access, and community workshops.' THEN '"يروج للزراعة الحضرية والوصول إلى الطعام وورش العمل المجتمعية."'::jsonb
        WHEN description = 'Education Program that works with students to prepare for Exam Schools, offers one-on-one tutoring and other classes.' THEN '"برنامج تعليمي يعمل مع الطلاب لإعدادهم لمدارس الامتحانات، يقدم دروساً خصوصية فردية وفصولاً أخرى."'::jsonb
        ELSE ('"' || description || '"')::jsonb
      END
  );