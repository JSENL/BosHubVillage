...
function entry(loc, { lastmod, changefreq = 'weekly', priority = '0.6' } = {}) {
  return {
    loc: loc.startsWith('http') ? loc : `${SITE_URL}${loc.startsWith('/') ? loc : `/${loc}`}`,
    lastmod,
    changefreq,
    priority,
  };
 ...
 */
export async function fetchSitemapEntries(supabase = createSitemapSupabase()) {
  const urls = STATIC_PAGES.map((page) =>
    entry(page.path, { changefreq: page.changefreq, priority: page.priority })
  );
