  ...
  return {
    url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`
      <svg width="40" height="40" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="20" y="25" text-anchor="middle" fill="#dc2626" font-size="6" font-weight="bold">${priceText}</text>
      </svg>
    `),
...
export const createEventMarker = ({ event, map, position, onMarkerClick }: MarkerConfig): google.maps.Marker => {
  console.log('Creating marker for event:', event.title, 'at position:', position);
