...
import { Calendar, Building2, Newspaper } from 'lucide-react';
...
export const UserContributions = ({ userId }: UserContributionsProps) => {
  ...
  const { data: contributions, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Fetch events, businesses, and news created by this user in parallel
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {t('profile.contributions', 'Contributions')} ({contributions?.length || 0})
        </CardTitle>
      </CardHeader>
      <CardContent>
